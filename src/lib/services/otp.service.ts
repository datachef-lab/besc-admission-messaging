...
import { otpTable, type Otp } from "@/db/schema";
import { eq, and, gt, lt, desc } from "drizzle-orm";
import { findUserByEmail, findUserByWhatsapp } from "./user.service";
...
export async function createOtp(text: string) {
    ...
    try {
        ...
        await sendWhatsAppMessage(user.whatsapp, [code], "logincode");
    } catch (error) {
...
export async function findOtpById(id: number) {
    const result = await db.select().from(otpTable).where(eq(otpTable.id, id));
    return result[0];
}
...
export async function findOtpByCode(code: string) {
    ...
    const result = await db.select().from(otpTable).where(eq(otpTable.code, code));
    return result[0];
}
...
export async function deleteOtp(id: number) {
    const result = await db.delete(otpTable).where(eq(otpTable.id, id)).returning();
    return result[0];
}
...
export async function findAllOtps() {
    return await db.select().from(otpTable);
}
